...
The review-fix findings in PR-4 surface a concrete pattern: **eligibility state refetched right after a success signal can unmount the very UI that displays that success** — the post-submit eligibility check confirmed `already_reviewed`, which then switched `canReview` to `false`, which unmounted the form wrapper, which also unmounted the nested success confirmation. This matters for future eligibility-gated-UI features (gated checkout, gated uploads, etc.): success messaging must sit outside the eligibility-dependent branch, or be paired with a stale-state indicator that tells the user their refresh succeeded.

### Scope boundaries

- **PR-5/PR-6 (edit/delete own review) remain explicitly droppable and not built** in this batch, per the spec's own slice table. They are listed as optional (open question 5: "recommend keeping them") but not touched by this delivery.
...
`npm run lint:api` clean · `npm run test:api` **800+/800+** · `npm run test -w @hb/web` **900+/900+** · `npm run build` clean. Code review (pre-PR) caught the confirmation visibility bug and whitespace validation gap; both fixed before PR open.
