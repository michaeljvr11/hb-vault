...
**Tests:** API 950 / Web 1052 · Lint clean · Build clean, 10 prerendered routes · Both new pages and the rewired footer verified live in the dev server. Migration `1788172800000-UserTermsAcceptedAt` was **not** run against a database — the Docker daemon was down, so the live `commission_rates` value could not be read either and the vendor page publishes the migration-seeded 15%.

**Follow-ups:** LC-1 (entity facts) still blocks eleven placeholder tokens plus four new ones on the customs page and one on the vendor agreement. The terms gate is router-only — a caller holding an access token can still reach the API without an acceptance record; server-side enforcement is unbuilt. Re-verify the live commission rate and run the migration once Postgres is up.
